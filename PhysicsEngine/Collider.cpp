...
namespace pEngine
{
    ...
    void Collider::DispatchCollisionEvent(const CollisionInfo& info, const Collider& other) const
    {
        if (!info.isColliding || !info.shouldTriggerEvent) {
            return;
        }
        ...
        if (other.onCollision_) {
            CollisionInfo mirrored = info;
            mirrored.normal = -info.normal;
            other.onCollision_(mirrored, *this);
        }
    }

    bool Collider::ShouldResolvePhysicalCollision(const Collider& other) const
    {
        return physicsResponseEnabled_ && other.physicsResponseEnabled_;
    }

    bool Collider::ShouldDispatchTriggerEvent(const Collider& other) const
    {
        return triggerResponseEnabled_ && other.triggerResponseEnabled_;
    }

    CircleCollider::CircleCollider(pObject::RigidBody* rb, Vector2 offset, double radius)
        : Collider(rb), radius(radius)
    {
        this->offset = offset;
    }
    ...
    CollisionInfo CircleCollider::CheckCollisionWith(const CircleCollider& other) const
    {
        ...
        if (res.isColliding)
        {
            ...
            res.shouldTriggerEvent = ShouldDispatchTriggerEvent(other);
            res.shouldResolvePhysics = ShouldResolvePhysicalCollision(other);
        }
    ...
    BoxCollider::BoxCollider(pObject::RigidBody* rb, Vector2 offset, double width, double height)
        : Collider(rb), width(width), height(height)
    {
        this->offset = offset;
    }
    ...
    CollisionInfo BoxCollider::CheckCollisionWith(const BoxCollider& other) const
    {
        ...
        info.shouldTriggerEvent = ShouldDispatchTriggerEvent(other);
        info.shouldResolvePhysics = ShouldResolvePhysicalCollision(other);
